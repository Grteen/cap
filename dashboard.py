...
class DashboardApp(QMainWindow):
    ...
    def init_column2(self):
        ...
        self.lbl_eta = QLabel('STEP 추종: 대기 중')
        ...
        hbox_virtual_delay = QHBoxLayout()
        hbox_virtual_delay.addWidget(QLabel('가상 API 지연(초):'))
        self.spin_virtual_delay = QDoubleSpinBox()
        self.spin_virtual_delay.setRange(0.0, 30.0)
        self.spin_virtual_delay.setDecimals(1)
        self.spin_virtual_delay.setSingleStep(0.1)
        self.spin_virtual_delay.setValue(self.virtual_delay_sec)
        self.spin_virtual_delay.valueChanged.connect(self._on_virtual_delay_changed)
        hbox_virtual_delay.addWidget(self.spin_virtual_delay)
        hbox_virtual_delay.addStretch(1)
        vbox.addLayout(hbox_virtual_delay)

        self.lbl_virtual_delay = QLabel('적용 지연: +0.0초')
        self.lbl_virtual_delay.setFont(QFont('Arial', 11, QFont.Weight.Bold))
        self.lbl_virtual_delay.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.lbl_virtual_delay.setStyleSheet('color: #03A9F4; padding: 4px;')
        vbox.addWidget(self.lbl_virtual_delay)
        vbox.addSpacing(6)
    ...
    def _on_virtual_delay_changed(self, value: float):
        self.virtual_delay_sec = float(value)
        self.lbl_virtual_delay.setText(f'적용 지연: +{self.virtual_delay_sec:.1f}초')
    ...
    @pyqtSlot(int, float, tuple, str)
    def add_classification_result(self, result_flag, latency, coeffs, raw_text):
        ...
        self.lbl_eta.setText('STEP 추종: 이벤트 처리 완료')
    ...
    def reset_status_labels(self):
        ...
        self.lbl_eta.setText('STEP 추종: 대기 중')
        self.lbl_eta.setStyleSheet('color: #4CAF50; border: 2px solid #4CAF50; padding: 10px;')
